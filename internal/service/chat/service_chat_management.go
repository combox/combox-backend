...
func (s *Service) ListMembers(ctx context.Context, userID, chatID string) ([]ChatMember, error) {
	userID = strings.TrimSpace(userID)
	...
	items, err := s.chats.ListChatMembers(ctx, chatID)
	if err != nil {
		return nil, internal(err)
	}
	return items, nil
...
func (s *Service) AddMembers(ctx context.Context, userID, chatID string, memberIDs []string) ([]ChatMember, error) {
	...
	if s.invites != nil {
		...
		return s.ListMembers(ctx, userID, chatID)
	}
...
func (s *Service) UpdateMemberRole(ctx context.Context, actorUserID, chatID, targetUserID, role string) ([]ChatMember, error) {
	...
	return s.ListMembers(ctx, actorUserID, chatID)
}
